...
export default function RoutesApp() {
    ...
    // Segundo Hook - useState (armazenar em um array os dados do LocalStorage)
    const [Data, setData] = useState([]);

    // Terceiro Hook - useEffect (pegar dados do LocalStorage na primeira inicialização da página)
    useEffect(() => {
    ...
    // Quarto Hook - useEffect (atualizar o LocalStorage toda vez que o array de dados é alterado)
    useEffect(() => {
        if (Data.length > 0) {
    ...
    // Quarto Hook - useEffect (atualizar o array de dados > localstorage toda vez que o array de receitas customizadas é alterado)
    useEffect(() => {
        if (CustomRecipe.id !== '') {
    ...
    const inserirItemNaLista = () => {
        ...
        const ingredient_raw = document.getElementById("ingredient-input").value.split(", ");
